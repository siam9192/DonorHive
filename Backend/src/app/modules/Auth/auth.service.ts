...
import { IAuthUser, IChangePasswordPayload, ILoginPayload, IRegistrationPayload } from "./auth.interface"
import ejs from "ejs";
 ...
 session.startTransaction()
  ...
  try {
    const createdRequest = await RegistrationRequest.create([{
      ...
      const token =  jwtHelpers.generateToken(tokenPayload,envConfig.jwt.accountVerificationTokenSecret as string,'10m');
     ...
     console.log(token)
      const emailSendStatus = await ejs.renderFile(
        path.join(
          process.cwd(),
          "/src/app/email-templates/registration-verification.html",
        ),
...
const verifyRegistration = async (token:string)=>{
  ...
  let decode;
  try {
   decode = await jwtHelpers.verifyToken(token,envConfig.jwt.accountVerificationTokenSecret as string) as {
      requestId:string,
      email:string
    };
  ...
  if(!decode) throw new AppError(httpStatus.BAD_REQUEST,"Maybe this link is expired or already used");
  ...
  if(!request) throw new AppError(httpStatus.BAD_REQUEST,"Maybe this link is expired or already used");
