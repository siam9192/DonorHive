...
import { useChangeUserStatusMutation, useDeleteUserMutation, useGetUserForManageQuery } from '../../redux/features/user/user.api';
...
const UserDetails = ({id,closePopup}:IProps) => {
  ...
  return (
    <div>     <h1 className="text-2xl font-medium text-gray-800">User Details</h1>
    <div className="mt-8">
      <div>
        <img
      ...
      <div className="mt-4">
        ...
        <div className="mt-3 space-y-3 font-secondary">
          ...
          <p className=" font-medium space-x-2 md:text-[1rem] text-sm">
            ...
            <span className={`${user.status ===  EUserStatus.Active ? 'text-primary':'text-red-500'} font-medium `}>{user.status}</span>
          </p>
          <p className=" font-medium space-x-2 md:text-[1rem] text-sm">
          ...
          <p className=" font-medium space-x-2 md:text-[1rem] text-sm">
            ...
            <span>
              {new Date(user.updatedAt).toDateString()}-{new Date(user.updatedAt).toLocaleTimeString()}
            </span>
          </p>
        </div>
      </div>
    ...
    {
        (status||user.status) ===  EUserStatus.Active &&   <button onClick={()=>handelChangeStatus(EUserStatus.Blocked)} className="p-2 bg-gray-100 rounded-full text-2xl text-pink-700">
        <GoBlocked />
      </button>
    }
    {
        (status||user.status) === EUserStatus.Blocked &&   <button onClick={()=>handelChangeStatus(EUserStatus.Active)} className="p-2 bg-gray-100 rounded-full text-2xl text-primary">
        <FaCheck />
      </button>
    }
    </div></div>
