  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-between p-2 lg:hidden">
        <h2 className="text-xl font-medium">Profile Menu</h2>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="  p-2 bg-gray-50 rounded-md text-2xl  top-2  right-1 rotate-2"
        >
          <AiOutlineMenu />
        </button>
      </div>
      <div
        className={`bg-white  flex flex-col justify-between lg:h-full lg:p-5 ${isOpen ? "  md:h-[800px] p-5" : "h-0 overflow-hidden"} duration-200 transition-all`}
      >
        <div>
          ...
          <div className="mt-10 ">
            {profileRoutes.map((route) => (
              <Link to={route.href} className=" group route px-3 py-5 flex  items-center gap-2 ">
                ...
                <span
                  className={`group-hover:text-primary md:text-xl text-lg font-medium ${pathname === route.href ? "text-primary" : "text-black"}`}
                >
                  {route.name}
                </span>
              </Link>
