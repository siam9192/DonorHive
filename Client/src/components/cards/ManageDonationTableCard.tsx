...
const ManageDonationTableCard = ({donation}:IProps) => {
  ...
  const donorType =  guestDonorInfo ? 'Guest':donation.isAnonymously ? 'Anonymously':'Registered'
    ...
    <tr className="odd:bg-white  even:bg-gray-50 e border-b  border-gray-200">
      ...
      >
        <div className="flex items-center gap-1">
          <img
            src={donation.campaign.coverImageUrl}
            alt=""
            className="size-12"
          />
      ...
      <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap ">
        <div className="">
          ...
          <p className="p-1 bg-green-100  rounded-full size-fit text-[0.8rem]">
            {donorType}
          </p>
      ...
      <td className="px-6 py-4 text-gray-900">{createdAt.toLocaleDateString()}-{createdAt.toLocaleTimeString()}</td>
      <td className="px-6 py-4 text-gray-900">{updatedAt.toLocaleDateString()}-{updatedAt.toLocaleTimeString()}</td>
